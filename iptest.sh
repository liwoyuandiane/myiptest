...
#	iconv -f UTF-8 -t GBK "$speedfile" > "$speedfile-gbk.csv"
    rm -f ./latest.csv
    iconv -f UTF-8 -t GBK "$speedfile" > "latest-gbk.csv"
#    echo -e "${Font_Green}测速文件:${Font_Red}$speedfile"
	echo -e "${Font_Green}测速文件: latest-gbk.csv"
else
...
echo -e "优选IP文件: realip-yxip.txt"
echo -e "${Font_Green}耗时:$[$end-$start]秒${Font_Suffix}"
rm -f "$speedfile" realip-$timestamp.txt
echo -e "#########################################"
echo -e "正在解析最优IP到域名..."
echo -e "#########################################"
chmod +x cfip_update.sh
./cfip_update.sh  realip-yxip.txt
